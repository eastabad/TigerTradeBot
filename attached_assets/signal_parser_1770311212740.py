...
class SignalParser:
    ...
    def _normalize_signal(self, signal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if quantity_str:
            ...
            if quantity_str == 'all' or normalized.get('is_close_signal'):
        ...
        # Stop loss and take profit (round to 2 decimal places for tick size compliance)
    ...
    def _validate_signal(self, signal: Dict[str, Any]) -> None:
        ...
        # Validate quantity (skip for close signals where quantity is 'all')
        if signal.get('close_all') or signal['quantity'] == 'all':
            pass  # Close all position - quantity will be determined from current position
